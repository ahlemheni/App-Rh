...
// Opération de suppression d'une sanction
if (isset($_GET['delete']) && !empty($_GET['delete'])) {
    $id = $_GET['delete'];
    $sql_delete = "DELETE FROM sanctions WHERE id_sanction = $id";
    if ($conn->query($sql_delete) === TRUE) {
        echo '<script>alert("Sanction supprimée avec succès"); window.location.href = "liste_sanctions.php";</script>'; 
        exit(); // Assurez-vous qu'aucun autre code PHP ne s'exécute après la redirection
    } else {
        echo "Erreur lors de la suppression de la sanction : " . $conn->error;
    }
}

// Récupération de la liste des sanctions depuis la base de données
                    ...
                    <?php if ($result->num_rows > 0): ?>
                        <?php while($row = $result->fetch_assoc()): ?>
                            <tr>
                                ...
                                <td><?php echo $row["date_sanction"]; ?></td>
                                <td><?php echo $row["commentaire"]; ?></td>
                                <td>
                                <a href="modifier_sanction.php?id=<?php echo $row['id_sanction']; ?>" class="edit-btn">Modifier</a>
                                <br>
                                <br>
                                    <a href="?delete=<?php echo $row['id_sanction']; ?>" class="delete-btn" onclick="return confirm('Êtes-vous sûr de vouloir supprimer cette sanction?')">Supprimer</a>
                                </td>
                            </tr>
                        <?php endwhile; ?>
                    <?php else: ?>
                        <tr>
                            <td colspan="8">Aucune sanction trouvée</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
